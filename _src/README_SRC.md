...
| `ITTS_OUT=<out> node _checkmuc.js` | `CHECKMUC OK: N ca · M tieu chi` - **MUC LUC CONG HOC VIEN CO DI CUNG MOT NHIP VOI THAN TRANG KHONG** (26/08, bo thu 48). Anh Luan: *"e nen xem lai ca cach bo tri menu, a thay no cu chay len chay xuong"*. Do bang cach cuon THAT bang con lan chuot tu dau toi cuoi trang roi ghi lai muc nao dang sang: duong di ra `3 4 0 5 6 7 1 10 13 8 9 2` - **lui nguoc 4 lan**. Goc KHONG nam o phep do cuon ma o cho **muc luc xep theo NHU CAU** (`HVGRP`) con **than trang xep theo THU TU VIET MA**; du phep do cuon co dung tuyet doi thi vet sang van nhay. Da chua bang `hvSec`/`hvXep` (HVGRP thanh thu tu DUY NHAT cho ca hai), va bo nay canh cho luat ay khoi troi - vi *mot luat vua dat ma chua co bo kiem thi no chi song bang tri nho cua nguoi vua dat ra no*. Nam cau: **M1** thu tu muc luc = thu tu than trang (do tren DOM that theo `offsetTop`) · **M2** cuon xuong deu thi vet sang khong LUI NGUOC · **M2b** vet sang co DI theo trang (dung mot cho suot ca trang nghia la phep do cuon da chet, ma chet kieu ay thi M2 van xanh) · **M3** moi muc tro toi mot khoi co that · **M4** moi khoi dang hien deu co duong toi. Do tren 3 ho so + che do phu huynh, vi moi ho so an/hien mot tap khoi khac nhau. | **BAY DA CAN NGAY LUOT DAU, VA LA BAY CUA CHINH BO KIEM:** no doi ho so ma KHONG tai lai trang, nen dia chi con mang muc cuoi cua ho so truoc va `bootHV` khoi phuc dung theo do - phep do ket luan "lui nguoc" trong khi app hoan toan dung. *Mot bo kiem di duong tat thi no do chinh duong tat ay, khong do duong nguoi dung di.* Nay xoa dia chi truoc moi ca. |
...
## 24/08 - M9 "CO CUM": CAT CHU TRONG KHI CHINH HANG AY CON BO TRONG CHO

Anh Luan gui anh man Cham bai: *"thiet ke no bi co cum lai thi phai"*. Do o 1440px: moi ten hoc
vien, chip trang thai va gio nop deu cut (thieu 8-31px), o diem hien **"Banc"** thay vi
"Band 0-9", **nua phai man hinh bo trong** - ca hang chi dung **63%** be ngang.

**Goc cua loi khong nam o man Cham bai.** `.rost .rn{flex:0 0 300px}` dat 14/08 cho man **Diem
danh** - o do cot ten chi chua ten cong mot chip ngan, chot 300px la de moi dong dung mot dong.
Man Cham bai muon nguyen class ay roi nhet **BA thu** vao cung 300px.
*Mot luat dat cho man nay ap sang man kia ma khong hoi lai noi dung cua no thi no bop chet noi
dung ay.*

### Vi sao ba phep do cu deu xanh - va day la cho dang hoc

| Phep do | No hoi gi | Vi sao khong dinh |
|---|---|---|
| `batNat` | "trinh duyet co dang cat chu khong" | **CO thay**, nhung khong biet ben canh con thua cho -> bao y het mot man that su chat, doc xong khong biet nen noi bo cuc hay rut gon noi dung |
| **M3** | "**hang chua no con thua cho khong**" | **Co san dung cau hoi**, nhung chi nguoc len dung mot nac `.sp,.tbar,.tbtren,.fld,.phead`. Trong mot dong danh sach, cha cua o **chinh la cai dong**, khong mang lop nao trong so do -> ve "con thua" **chua bao gio chay** |
| M6 | "chu bi bop thanh cot hep" | O day chu khong xuong dong, no bi **cat cut** |

*Co san cau hoi dung ma hoi sai cho thi cung bang khong hoi.*

**Cau hoi cua M9:** mot dong flex (`.rost`, `.lrow`, `.obrow`) ma trong no co o dang cat chu, VA
be rong dong tru tong be rong cac cot va cac khe con doi tu **60px** tro len -> do. Bo qua dong
da xuong hang (kho hep bat `flex-wrap`) vi luc ay cong be rong theo hang ngang la vo nghia.

## VA MOT TRANG CO TEN TRONG DANH SACH KHONG CO NGHIA LA MOI THU NO VE RA DEU DUOC NHIN

`_checkmat` **co** di qua trang `baitap`. Nhung `go("baitap")` chi mo nac mac dinh **Giao bai**;
ba nac con lai (**Thu bai · Cham bai · Cho cham**) la ba bo cuc khac han va **chua nac nao tung
bi do**.

Day la **lan thu BA** cung mot cai bay, chi doi hinh:
- 06/08 thieu **TRANG** `viec` (khoi Nhip ngay chi nam o do);
- 09/08 thieu **TRANG** `bangcong` (o tien bi be doi chi nam o do);
- 24/08 thieu **NAC ben trong mot trang da co ten trong danh sach**.

Hai lan truoc em ket luan "danh sach phai phu du KIEU BO CUC" roi van dem bo cuc theo **TEN
TRANG**. Nay muc trong `TRANG` khai duoc `dat` - cau lenh chay ngay truoc `go()`:

```js
{k: "baitap", ten: "baitap/cham", dat: "window.BTMODE='cham'"}
```

Them ca man `diemdanh`, vi hai man song chung mot luat CSS thi phai do cung nhau.

**Luot chay DAU TIEN do ngay o man Thu bai:** bo trong **507px tren 1100px** ma ten, chip va han
nop deu cut. Cung mot benh, cung mot trang, chi khac cai nut o tren - va no nam do suot.
*Va mot cho theo mot luat thi phai di hoi moi cho khac dang song duoi luat ay* - lan nay cai
thuoc di hoi ho, nen cho thu hai lo ra trong mot phut thay vi cho anh Luan gui them mot anh.

**M9 khong can co tinh pha de chung minh** (nhu M8/M8b da phai lam) - no do that ngay luot dau,
o mot cho chua ai chi.

Kho dien thoai lo them hai cho: chip vo 3 dong o man Thu bai, va o Nhan xet chi con **66px** o
man Cham bai (o diem va o "nop the nao" an het 336px). *Mot o de viet nhan xet ma hep hon cai
nhan cua chinh no thi no khong con la o nhap nua.*

Sau khi va: `_checkmat` **24 trang x 2 kho man, 3090 chuoi - xanh**.

---

## 24/08 - `_checkbc`: MOI GHE MOT BAN BAO CAO (bo kiem thu 46)

Anh Luan: *"trang tong quan, bao cao va KPI hinh nhu moi chuc danh lai la 1 cach thiet ke khac
nhau moi dung em. Chu em dung khung chung, cai nao co thi hien, ko co thi an anh thay ko hop ly,
ko sat voi nghiep vu dau"*.

**45 bo kiem khong bo nao do.** Doc lai thi thay vi sao: chung hoi *"trang co ve duoc khong"*,
*"co ro du lieu mien khac khong"*, *"chu co bi cat khong"*. Khong bo nao hoi *"trang nay co phai
ban bao cao CUA NGUOI NAY khong"*. **Mot khung chung khong ro du lieu, khong gay, khong cat chu -
no chi SAI NGUOI, va cai sai ay khong co hinh dang nao de bat.**

### Bay cua chinh bo kiem, can ngay luot dau

Bo nay chi goi `applyScope(sid)` roi do. `applyScope` dat dung NHOM va MUC mien nen `bcKey()`,
`dsLevel()` deu tra loi dung va **moi thu trong nhu dang chay**. Nhung `CURSTAFF` thi do
`setRole()` dat (no doc `window.GATE_SID`) - thieu buoc ay thi CURSTAFF van la **"ADMIN"**, va
moi phep so "cua minh" trong `canRow` dem chu so huu ra so voi "ADMIN" nen truot het:
`srows("DL09")`, `srows("DL11")` tra ve **0 cho MOI chuc danh**.

Em da doc con so 0 ay thanh *"app khong gan lop cho giao vien"* - trong khi giao vien NV005 day
**70 buoi** co that. Do lai bang `gateEnter()`: Giao vien ACA co 13 HV, 2 lop, 29 buoi, 61 bai,
30/36 chi so tinh duoc.

*Do mot thu dang bi che thi moi con so deu la 0, va so 0 trong y het mot ket qua.*

Nay co **luat B0**: ngoi xuong roi phai hoi lai xem minh da ngoi dung ghe chua.
*Mot bo kiem khong kiem chinh cai thuoc cua no thi no do bang niem tin.*
(`_checkghe` lam dung tu dau: `applyScope` roi `setRole`. Da doi chieu.)

### Va mot luat KHONG BAO GIO DO DUOC - da phai thay

Ban dau **B3** hoi *"thu tu ve ra co khop thu tu da khai khong"*. Thu pha bang cach dao han dan
bai Hoc vu: **van xanh**. Dung thoi - bo dung doc ban khai roi ve theo ban khai, nen dao ban khai
la dao luon thu tu ve, hai ve luc nao cung bang nhau.
Dung cai bay da ghi tu 14/08: *do cai ong minh vua noi thi bao gio cung thay thong.*

Thay bang hai cau **doc lap voi bo dung**:
- **B3a** khoi nao ve ra cung phai co ten trong dan bai cua ghe - bat duoc ca se tai dien that:
  ai do them thang `h+=deptSection()` vao `baocaoThan`, di vong qua `BCMAU`, **khung chung moc
  lai tu chinh cho no vua bi go**;
- **B3b** khoi DAU TIEN phai thuoc chu de cua ghe. *Thu tu khong phai trang tri - thu dung dau
  la thu nguoi ta doc.*

### Sau luat, nam luat da thu pha va deu do dung cho

| Luat | Hoi gi | Da pha bang cach |
|---|---|---|
| B0 | ngoi dung ghe chua | (tu kiem, khong can pha) |
| B1 | moi ghe co dan bai rieng khong | xoa dan bai Nhan su -> 3 cho do |
| B2 | dan bai co ten va cau hoi khong | (cau truc) |
| B3a | co khoi la chen vao khong | goi thang `deptSection()` -> 12 cho do |
| B3b | mo dau bang chu de cua ghe khong | dao thu tu, KPI len dau -> do |
| B4 | co khoi cua mien la khong | them `pheu` vao Nhan su, bo `can` -> 3 cho do |
| B5 | co bang rong vi khong co gi de dem khong | tra `hvnguyco` ve dieu kien cu -> 6 cho do |
