...
| `ITTS_OUT=<out> node _tall.js` | `Render 39 trang \| 0 loi` + `Icon dung: 198 \| thieu trong font: khong` (từ V9.40 icon thiếu là ĐỎ THẬT) |
...
| `ITTS_OUT=<out> node _checkui.js` | `CHECKUI OK: da mo THAT 491 luot` - **kiểm thử trên trình duyệt thật** (cần `npm i playwright` một lần; máy không có Chromium thì tự BỎ QUA chứ không báo đỏ bậy) |
